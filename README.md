...
Ink is per-slide and clears automatically when you change slides.
...
产物：`index.html`（全屏放映页，点右下角 **▶ Fullscreen** 即全屏；左下角工具条可用**激光指针 / 画笔 / 橡皮 / 清除**，快捷键 `L`/`P`/`E`/`Del`，切页自动清除笔迹）、`presenter.html`（演讲者视图：当前页 + 下一页 + 备注 + 计时器，双屏同步）、本地字体/图片/GIF，断网也能放。原理：先用无头 Chrome 把带模板的 deck 渲染展开并快照，再把字体和素材本地化、动画在切到该页时重新播放。按钮文字可用 `--button-label` 改（如 `--button-label "▶ 全屏放映"`）。
