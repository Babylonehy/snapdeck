...
| File | What it is |
|------|------------|
| `index.html` | The deck — full-bleed, keyboard nav (`←/→/Space/digits`), animations & GIFs, a **▶ Fullscreen** button, and on-slide **annotation tools** (laser pointer + pen + eraser). Works by double-click, **fully offline**. |
...
The playback page has a small toolbar (bottom-left) for drawing on slides during your talk:

| Tool | Shortcut | What it does |
|------|----------|--------------|
| **Cursor** | `Esc` | Normal mode — clicks/keys drive the deck. |
| **Laser** | `L` | A glowing red dot follows your mouse. |
| **Pen** (red / yellow / blue) | `P` | Freehand draw on the slide. |
| **Erase** | `E` | Rub out part of your ink. |
| **Undo** | `Z` / `⌘`·`Ctrl`+`Z` | Step back one stroke (or undo a Clear). |
| **Clear** | `Del` | Wipe all ink on the slide. |

The toolbar **auto-hides when idle** (like the page counter) so it never blocks the slide — just move the mouse to bring it back — and you can **drag it by the `⠿` grip** to reposition it. Ink is per-slide and clears automatically when you change slides.
...
产物：`index.html`（全屏放映页，点右下角 **▶ Fullscreen** 即全屏；工具条可用**激光指针 / 画笔 / 橡皮 / 撤销 / 清除**，快捷键 `L`/`P`/`E`/`Z`·`⌘Z`/`Del`，空闲时自动隐藏、移动鼠标再出现、可拖 `⠿` 手柄换位置，切页自动清除笔迹）、`presenter.html`（演讲者视图：当前页 + 下一页 + 备注 + 计时器，双屏同步）、本地字体/图片/GIF，断网也能放。原理：先用无头 Chrome 把带模板的 deck 渲染展开并快照，再把字体和素材本地化、动画在切到该页时重新播放。按钮文字可用 `--button-label` 改（如 `--button-label "▶ 全屏放映"`）。
